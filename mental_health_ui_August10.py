...
FACE_MODEL_PATH   = r"E:\HK8\PBL4\Code\FolderGithub\resnettop1.pth"
...
if os.path.exists(FACE_MODEL_PATH):
    try:
        ...
        # 1. Gỡ bỏ module. nếu mô hình từng huấn luyện bằng DataParallel
        state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
        
        # 2. Tự động bổ sung tiền tố resnet. nếu cấu trúc lớp bị lệch tên
        first_key = list(state_dict.keys())[0]
        if not first_key.startswith('resnet.'):
            state_dict = {f'resnet.{k}': v for k, v in state_dict.items()}
            
        # 3. 🛠️ FIX CHÍ MẠNG: Đồng bộ hóa cấu trúc tầng phân loại (Chuyển đổi fc.1 về fc)
        fixed_state_dict = {}
        for k, v in state_dict.items():
            new_key = k
            if "resnet.fc.1." in k:
                new_key = k.replace("resnet.fc.1.", "resnet.fc.")
...
with gr.Blocks(title="Mental Health Monitor", css=CSS) as app:
    ...
    with main_app:
      with gr.Tabs():
        ...
        # ── TAB 2: KHUÔN MẶT (Tích hợp Ảnh tĩnh & Video) ─────────────────────────
        with gr.Tab("📷 Khuôn mặt"):
            gr.HTML(
                ...
                f'<div style="color:#8888aa;font-size:12px">'
                f'Lựa chọn chụp ảnh tĩnh hoặc quay video (quét 5 FPS) để phân tích biểu cảm.</div></div>'
            )
            
            # --- KHU VỰC INPUT (Chia 2 Tab con) ---
            with gr.Tabs():
                with gr.Tab("📸 Ảnh tĩnh"):
                    face_img_input = gr.Image(label="Camera / Upload Ảnh", sources=["webcam", "upload"], type="numpy")
                    face_img_btn = gr.Button("🔍 Phân tích Ảnh", variant="primary")
                    
                with gr.Tab("🎬 Video"):
                    face_vid_input = gr.Video(label="Camera / Upload Video", sources=["webcam", "upload"])
                    face_vid_btn = gr.Button("🔍 Phân tích Video", variant="primary")
                    face_status = gr.Textbox(label="Trạng thái xử lý Video", interactive=False)

            # --- KHU VỰC OUTPUT (Dùng chung cho cả Ảnh và Video) ---
            gr.HTML('<div style="margin-top: 16px; border-top: 1px solid #2a2a4e; padding-top: 16px;"></div>')
            with gr.Row():
                with gr.Column(scale=2):
                    face_out = gr.Image(label="📸 Khung hình kết quả", interactive=False)
                with gr.Column(scale=3):
                    face_label = gr.HTML()
                    face_bars  = gr.HTML()
            face_suggestion = gr.HTML()

            # ==========================================
            # HÀM XỬ LÝ 1: ẢNH TĨNH
            # ==========================================
            def handle_face_image(img, user_name, session_records, history):
                if img is None:
                    return "", None, "<div style='color:#F5A623;padding:10px;background:#1a1a2e;border-radius:8px'>⚠️ Chưa có ảnh</div>", "", "", session_records, history
                if not face_available:
                    return "", None, "<div style='color:#E84040;padding:10px'>⚠️ Face model chưa load</div>", "", "", session_records, history
                
                try:
                    img_arr = np.array(img)
                    if img_arr.ndim == 2: img_arr = cv2.cvtColor(img_arr, cv2.COLOR_GRAY2RGB)
                    elif img_arr.shape[2] == 4: img_arr = img_arr[:, :, :3]
                    frame = cv2.cvtColor(img_arr, cv2.COLOR_RGB2BGR)
                    
                    probs, bbox = predict_face_single(frame)
                    if not probs:
                        return "", img_arr, "<div style='color:#aaa;padding:12px;background:#1a1a2e;border-radius:10px'>😶 Không phát hiện khuôn mặt</div>", "", "", session_records, history
                    ...
                    if bbox: frame = draw_face_label(frame, bbox, f'{vi} {conf:.0%}')
                    ...
                    if user_name.strip():
                        record = {
                            ...
                            'text': '', 'text_emotion': None, 'text_conf': 0.0,
                            'face_emotion': emotion, 'face_conf': conf,
                            'final_emotion': emotion, 'final_conf': conf,
                            'w_text': 0.0, 'w_face': 1.0, 'conflict': False,
                        }
                        session_records = session_records + [record]
                        history = load_history()
                        history.setdefault(user_name, {}).setdefault(str(date.today()), []).append(record)
                    ...
                    return "✅ Phân tích ảnh tĩnh hoàn tất!", out_img, label_html, make_bar_html(probs), make_suggestion_html(emotion), session_records, history
                except Exception as e:
                    return f"❌ Lỗi: {e}", None, f"<div style='color:#E84040'>❌ Lỗi: {e}</div>", "", "", session_records, history

            # ==========================================
            # HÀM XỬ LÝ 2: VIDEO (5 FPS)
            # ==========================================
            def handle_face_video(video_path, user_name, session_records, history):
                _KEEP = gr.update()
                if not video_path:
                    yield "⚠️ Vui lòng quay hoặc tải video lên!", None, "<div style='color:#F5A623;padding:10px;background:#1a1a2e;border-radius:8px'>⚠️ Chưa có video</div>", "", "", session_records, history
                    return
                if not face_available:
                    yield "⚠️ Face model chưa load", None, "<div style='color:#E84040;padding:10px'>⚠️ Face model chưa load</div>", "", "", session_records, history
                    return

                yield "⏳ Đang quét chuỗi thời gian khuôn mặt (5 FPS)...", _KEEP, _KEEP, _KEEP, _KEEP, session_records, history
                ...
                try:
                    ...
                    while cap.isOpened():
                        ...
                        if frame_idx % interval == 0:
                            probs, bbox = predict_face_single(frame_bgr)
                            if probs:
                                all_face_probs.append(probs)
                                current_top = max(probs, key=probs.get)
                                if probs[current_top] > max_conf:
                                    max_conf = probs[current_top]
                    ...
                    if not all_face_probs:
                        yield "😶 Không phát hiện khuôn mặt", None, "<div style='color:#aaa;padding:12px;background:#1a1a2e;border-radius:10px'>😶 Không phát hiện khuôn mặt</div>", "", "", session_records, history
                        return
                    ...
                    conf = face_probs[emotion]
                    col = EMOTION_COLOR[emotion]
                    vi  = EMOTION_VI[emotion]

                    labeled = draw_face_label(best_frame, best_bbox, f'{vi} {conf:.0%}')
                    out_img = cv2.cvtColor(labeled, cv2.COLOR_BGR2RGB)
                    ...
                    label_html = (
                        ...
                        f'<div style="font-size:40px">{vi.split()[0]}</div>'
                        f'<div style="font-size:20px;font-weight:700;color:{col}">{" ".join(vi.split()[1:])}</div>'
                        f'<div style="color:#aaa;font-size:14px;margin-top:4px">Độ tin cậy tổng hợp: {conf:.0%}</div></div>'
                    )
                    ...
                    yield f"✅ Phân tích video hoàn tất! Cảm xúc: {vi} ({conf:.0%})", out_img, label_html, make_bar_html(face_probs), make_suggestion_html(emotion), session_records, history

                except Exception as e:
                    yield f"❌ Lỗi xử lý: {e}", None, f"<div style='color:#E84040'>❌ Lỗi: {e}</div>", "", "", session_records, history

            # ==========================================
            # KẾT NỐI BUTTONS VỚI CÁC HÀM XỬ LÝ
            # ==========================================
            face_img_btn.click(
                fn=handle_face_image,
                inputs=[face_img_input, user_name_input, session_state, history_state],
                # Ảnh tĩnh cũng xuất ra face_status để ghi đè chữ "Đang chạy video..." nếu có
                outputs=[face_status, face_out, face_label, face_bars, face_suggestion, session_state, history_state]
            )
            
            face_vid_btn.click(
                fn=handle_face_video,
                inputs=[face_vid_input, user_name_input, session_state, history_state],
                outputs=[face_status, face_out, face_label, face_bars, face_suggestion, session_state, history_state]
            )
        ...
        # ══════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════
        # TAB 3: ĐA PHƯƠNG THỨC (VIDEO: GIỌNG NÓI + KHUÔN MẶT) - V8
        # ══════════════════════════════════════════════════════════════
        with gr.Tab("🔀 Đa phương thức (Video)"):
            gr.HTML('''<div style="background:#1a1a2e;border-radius:10px;padding:14px 16px;margin:8px 0;border:1px solid #2a2a4e">
              <div style="color:#a78bfa;font-weight:700;font-size:14px;margin-bottom:8px">🎬 Đánh giá Toàn trình (Video)</div>
              <div style="color:#8888aa;font-size:13px">Hệ thống hỗ trợ quay video trực tiếp bằng Camera hoặc Tải lên video sẵn có (.mp4).</div></div>''')
            
            with gr.Row():
                with gr.Column(scale=2):
                    # Sử dụng Component Video chuẩn của Gradio (Hỗ trợ Webcam + Upload)
                    mm_video = gr.Video(label="🎬 Quay / Upload Video", sources=["webcam", "upload"])
                    mm_btn = gr.Button("🔍 Phân tích Video", variant="primary", size="lg")
                    mm_status = gr.Textbox(label="Trạng thái hệ thống", interactive=False)
                
                with gr.Column(scale=3):
                    mm_face_img = gr.Image(label="📸 Khung hình phân tích tốt nhất", interactive=False)
                    mm_lbl = gr.HTML()
                    mm_bars = gr.HTML()

            gr.HTML('<div style="color:#a78bfa;font-weight:600;font-size:14px;margin:14px 0 6px">📝 Văn bản nhận dạng</div>')
            mm_text = gr.Textbox(label="", interactive=False)
            mm_suggestion = gr.HTML()

            def handle_mm_video(video_path, user_name, session_records, history):
                _KEEP = gr.update()
                if not video_path:
                    yield "⚠️ Vui lòng quay video hoặc upload file!", None, "", "", "", "", session_records, history
                    return

                yield "⏳ Đang trích xuất luồng âm thanh...", _KEEP, _KEEP, _KEEP, _KEEP, _KEEP, session_records, history
                
                # ─────────────────────────────────────────────────────────
                # 1. BÓC TÁCH AUDIO BẰNG FFMPEG LÕI (CHỐNG CRASH WEBM)
                # ─────────────────────────────────────────────────────────
                import tempfile
                import subprocess
                ...
                # Ép ffmpeg trích xuất audio ra file chuẩn 16kHz mono cho PhoWhisper
                subprocess.run([
                ...
                if not os.path.exists(temp_audio) or os.path.getsize(temp_audio) < 1000:
                    yield "⚠️ Video bị tắt tiếng hoặc không có âm thanh!", None, "", "", "", "", session_records, history
                    return

                # ─────────────────────────────────────────────────────────
                # 2. NHẬN DẠNG GIỌNG NÓI (PhoWhisper + XLM-R)
                # ─────────────────────────────────────────────────────────
                yield "⏳ Đang chạy PhoWhisper dịch giọng nói...", _KEEP, _KEEP, _KEEP, _KEEP, _KEEP, session_records, history
                try:
                    import librosa
                    speech_array, _ = librosa.load(temp_audio, sr=16000)
                    asr_result = asr_pipe({'array': speech_array, 'sampling_rate': 16000})
                    transcribed_text = re.sub(r'<\|[^|]+\|>', '', asr_result['text']).strip()
                    if not transcribed_text: transcribed_text = "😶 (Không phát hiện tiếng nói)"
                except Exception as e:
                    transcribed_text = f"⚠️ Lỗi dịch: {e}"
                
                text_probs = predict_text_full(transcribed_text)
                text_top = max(text_probs, key=text_probs.get)

                # ─────────────────────────────────────────────────────────
                # 3. QUÉT KHUÔN MẶT BẰNG OPENCV (TÌM BEST FRAME)
                # ─────────────────────────────────────────────────────────
                yield "⏳ Đang quét chuỗi thời gian khuôn mặt (5 FPS)...", _KEEP, _KEEP, _KEEP, _KEEP, _KEEP, session_records, history
                ...
                while cap.isOpened():
                    ...
                    if frame_idx % interval == 0:
                        ...
                        if probs:
                            ...
                            # Tìm frame có cảm xúc bộc lộ mạnh nhất
                            current_top = max(probs, key=probs.get)
                            if probs[current_top] > max_conf:
                                max_conf = probs[current_top]
                                best_frame = frame_bgr.copy()
                ...
                # Tổng hợp Xác suất Face
                if all_face_probs:
                    face_probs = {e: sum(p[e] for p in all_face_probs)/len(all_face_probs) for e in COMMON_EMOTIONS}
                    face_top = max(face_probs, key=face_probs.get)
                    conf_face = face_probs[face_top]
                    labeled = draw_face_label(best_frame, best_bbox, f"{EMOTION_VI[face_top]} {conf_face:.0%}")
                ...
                # ─────────────────────────────────────────────────────────
                # 4. DUNG HỢP LATE FUSION VÀ KẾT XUẤT UI
                # ─────────────────────────────────────────────────────────
                final_emotion, final_conf, fused_probs, w_text, w_face = fuse_emotions(text_probs, face_probs)
                ...
                detail = (
                    ...
                    + (f'<div style="color:#aaa;margin-top:4px">📷 Mặt: <span style="color:{EMOTION_COLOR.get(face_top,"#aaa")}">{EMOTION_VI.get(face_top,"—")} ({conf_face:.0%})</span></div>'
                       f'<div style="color:#aaa;margin-top:4px">⚖️ Trọng số: Speech={w_text:.2f} / Face={w_face:.2f}</div>' if face_probs else '<div style="color:#555;margin-top:4px">📷 Không thấy mặt</div>')
                    + ('<div style="color:#FF6B6B;margin-top:6px;font-weight:bold">⚠️ Phát hiện xung đột cảm xúc (Hidden Mood)!</div>' if conflict else '')
                    + '</div>'
                )
                
                lbl = (
                    f'<div style="text-align:center;padding:20px;background:#1a1a2e;border-radius:12px;border:2px solid {col}">'
                    f'<div style="font-size:42px">{vi.split()[0]}</div><div style="font-size:20px;font-weight:700;color:{col}">{" ".join(vi.split()[1:])}</div>'
                    f'<div style="color:#aaa;font-size:14px;margin-top:4px">Độ tin cậy: {final_conf:.0%}</div></div>{detail}'
                )
                
                record = {
                    'time': datetime.now().strftime('%H:%M:%S'), 'date': str(date.today()),
                    'text': transcribed_text, 'text_emotion': text_top, 'text_conf': text_probs[text_top],
                    'face_emotion': face_top, 'face_conf': conf_face if face_probs else 0.0,
                ...
                yield f"✅ Hoàn tất! Cảm xúc: {vi} ({final_conf:.0%})", face_display, transcribed_text, lbl, make_bar_html(fused_probs), make_suggestion_html(final_emotion), session_records, history
            mm_btn.click(fn=handle_mm_video, inputs=[mm_video, user_name_input, session_state, history_state], 
                         outputs=[mm_status, mm_face_img, mm_text, mm_lbl, mm_bars, mm_suggestion, session_state, history_state])
...
# python mental_health_ui_August10.py