...
FACE_MODEL_PATH   = r"E:\HK8\PBL4\Code\FolderGithub\nb_resnet_best.pth"
...
def handle_webcam_input(image, user_name):
    ...
    try:
        ...
        return (None,
                ...
                f"❌ Lỗi: {e}</div>",
                "", "")
def handle_video_input(video_path, user_name):
    """Đọc video clip, xử lý phòng vệ lỗi dừng sớm của WebM và tính trung bình cộng xác suất."""
    if video_path is None:
        return (
            "<div style='color:#F5A623;padding:10px;background:#1a1a2e;border-radius:8px'>"
            "⚠️ Chưa có dữ liệu video — hãy quay clip hoặc tải lên file video</div>", 
            "", ""
        )
    if not face_available:
        return "<div style='color:#E84040;padding:10px'>⚠️ Face model chưa load</div>", "", ""
    ...
    try:
        ...
        while cap.isOpened():
            try:
                ...
                # Tối ưu: Cứ sau 5 frames bóc 1 ảnh phân tích để giữ FPS cao
                if frame_count % 5 == 0:
                ...
                # Nếu file .webm bị kết thúc đột ngột giữa chừng, thoát vòng lặp và xử lý lượng dữ liệu đã có
                break
            
        cap.release()

        # Phòng vệ: Nếu có lỗi kết thúc sớm nhưng hệ thống kịp đọc > 0 khung hình thì vẫn xử lý tiếp
        if not all_probs:
            return (
                "<div style='color:#aaa;padding:12px;background:#1a1a2e;border-radius:10px'>"
                "😶 Không trích xuất được khuôn mặt hợp lệ từ đoạn clip.<br>"
                "<small style='color:#666'>Vui lòng thử quay lại, giữ camera đứng yên và nhìn thẳng bối cảnh.</small></div>", 
                "", ""
            )

        # Tính toán ma trận xác suất trung bình tích lũy theo thời gian
        avg_probs = {e: 0.0 for e in COMMON_EMOTIONS}
        for probs in all_probs:
            for e in COMMON_EMOTIONS:
                avg_probs[e] += probs[e]
        for e in COMMON_EMOTIONS:
            avg_probs[e] /= len(all_probs)

        emotion = max(avg_probs, key=avg_probs.get)
        col     = EMOTION_COLOR[emotion]
        vi      = EMOTION_VI[emotion]
        conf    = avg_probs[emotion]

        if not user_name.strip():
            user_name = "Người dùng"
            
        record = log_record(user_name, "[Phân tích qua đoạn Video Clip]", {e: 0.0 for e in TEXT_EMOTIONS}, avg_probs)

        label_html = (
            f'<div style="text-align:center;padding:16px;background:#1a1a2e;'
            f'border-radius:12px;border:2px solid {col};margin-top:8px">'
            f'<div style="font-size:14px;font-weight:700;color:#8888aa;margin-bottom:4px">📹 KẾT QUẢ PHÂN TÍCH CLIP</div>'
            f'<div style="font-size:40px">{vi.split()[0]}</div>'
            f'<div style="font-size:20px;font-weight:700;color:{col}">{" ".join(vi.split()[1:])}</div>'
            f'<div style="color:#aaa;font-size:13px;margin-top:4px">Độ tin cậy trung bình: {conf:.0%} (Đã phân tích thành công {len(all_probs)} khung hình)</div>'
            f'</div>'
        )
        bars       = make_bar_html(avg_probs)
        suggestion = make_suggestion_html(emotion)
        return label_html, bars, suggestion

    except Exception as e:
        return f"<div style='color:#E84040;padding:10px;background:#1a1a2e;border-radius:8px'>❌ Lỗi hệ thống video: {e}</div>", "", ""

from moviepy.video.io.VideoFileClip import VideoFileClip
import scipy.io.wavfile as wav

def handle_multimodal(text, audio, image, video_path, user_name):
    """Hàm dung hợp Đa phương thức cao cấp: Tự động trích xuất cả Tiếng và Hình từ 1 file Video duy nhất."""
    if not user_name.strip():
        user_name = "Người dùng"
        
    visual_source = "📷 Ảnh tĩnh"
    text_transcribed = ""
    face_probs = None

    # ══════════════════════════════════════════════════════════════
    # CHẾ ĐỘ 1: NGƯỜI DÙNG SỬ DỤNG VIDEO CLIP (TỰ ĐỘNG TÁCH TIẾNG + HÌNH)
    # ══════════════════════════════════════════════════════════════
    if video_path is not None and face_available:
        visual_source = "📹 Video Clip (Đồng bộ Lời nói + Nét mặt)"
        temp_audio_path = "temp_extracted_voice.wav"
        
        try:
            # --- Nhánh 1: Tách luồng âm thanh âm bản từ Video ---
            video_clip = VideoFileClip(video_path)
            if video_clip.audio is not None:
                # Ép xuất ra file wav chuẩn 16000Hz phục vụ PhoWhisper
                video_clip.audio.write_audiofile(
                    temp_audio_path, fps=16000, nbytes=4, 
                    codec='pcm_s16le', verbose=False, logger=None
                )
                video_clip.close() # Giải phóng file ngay lập tức
                
                # Đọc file wav tạm thời vừa trích xuất để nạp vào mô hình speech
                sr, arr = wav.read(temp_audio_path)
                text_transcribed = transcribe_from_array(arr, sr)
                
                # Dọn dẹp file nháp trên ổ cứng sau khi nhận dạng xong
                if os.path.exists(temp_audio_path):
                    os.remove(temp_audio_path)
            else:
                video_clip.close()
        except Exception as e:
            print(f"⚠️ Cảnh báo tách âm thanh video thất bại: {e}")
            text_transcribed = ""

        # Ưu tiên lấy văn bản dịch từ video; nếu video không có tiếng thì mới xét đến ô nhập text thủ công
        if text_transcribed and not text_transcribed.startswith("⚠️") and not text_transcribed.startswith("Lỗi"):
            text = text_transcribed
            
        # --- Nhánh 2: Quét chuỗi khung hình xử lý thị giác (Giữ nguyên thuật toán tối ưu của bạn) ---
        try:
            cap = cv2.VideoCapture(video_path)
            all_probs = []
            frame_count = 0
            ...
            if all_probs:
                ...
                for probs in all_probs:
                    for e in COMMON_EMOTIONS:
                        face_probs[e] += probs[e]
                for e in COMMON_EMOTIONS:
                    face_probs[e] /= len(all_probs)
        except Exception:
            face_probs = None

    # ══════════════════════════════════════════════════════════════
    # CHẾ ĐỘ 2: NGƯỜI DÙNG SỬ DỤNG ẢNH TĨNH VÀ AUDIO RỜI (LUỒNG CŨ)
    # ══════════════════════════════════════════════════════════════
    else:
    ...
    # Nếu cuối cùng quét cả hệ thống mà không thu được bất kỳ chữ nào
    if not text.strip():
        return "", "<div style='color:#aaa;padding:10px'>Vui lòng nhập chữ, nói vào video clip hoặc tải file audio lên</div>", "", "", ""
        
    # 3. Tính toán cơ chế Late Fusion (Giữ nguyên logic của bạn)
    record  = log_record(user_name, text, text_probs=predict_text_full(text), face_probs=face_probs)
    emotion = record['final_emotion']
    col     = EMOTION_COLOR[emotion]
    vi      = EMOTION_VI[emotion]
    ...
    detail  = (
        ...
        f'<div style="color:#aaa;margin-bottom:6px">📝 Ngôn ngữ (Dịch từ Video/Text): '
        f'<span style="color:{EMOTION_COLOR[record["text_emotion"]]}">'
        f'{EMOTION_VI[record["text_emotion"]]} ({record["text_conf"]:.0%})</span></div>'
        + (f'<div style="color:#aaa;margin-bottom:6px">{visual_source}: '
           ...
           f'<div style="color:#aaa;margin-bottom:6px">⚖️ Tỷ trọng dung hợp: '
           f'Ngôn ngữ={record["w_text"]:.2f} / Thị giác={record["w_face"]:.2f}</div>'
    ...
    label = (
        ...
        f'<div style="font-size:20px;font-weight:700;color:{col}">{" ".join(vi.split()[1:])}</div>'
        f'<div style="color:#aaa;font-size:14px">Độ tin cậy tổng hợp: {record["final_conf"]:.0%}</div>'
        f'</div>' + detail
    )
    
    fused_probs = {
        e: record['w_text'] * predict_text_full(text).get(e, 0) +
           record['w_face'] * (face_probs.get(e, 0) if face_probs else 0)
        for e in COMMON_EMOTIONS
    }
    
    bars       = make_bar_html(fused_probs)
    suggestion = make_suggestion_html(emotion)
    score_html = make_score_html(user_name)
...
# CHUẨN HÓA: Truyền tham số css=CSS vào đây để kích hoạt Dark Mode
       ...
       # ── TAB 2: KHUÔN MẶT ─────────────────────────────────────
        with gr.Tab("📷 Khuôn mặt"):
            gr.HTML(
                ...
                f'<div style="color:#8888aa;font-size:12px">Chọn chế độ: Chụp ảnh tĩnh từng khoảnh khắc hoặc Quay một đoạn Video ngắn (3-5 giây)</div>'
                f'</div>'
            )
            
            # Tạo hai lựa chọn xử lý ngay trong Tab Khuôn mặt
            with gr.Tabs():
                
                # --- CHẾ ĐỘ 1: ẢNH TĨNH ---
                with gr.Tab("🖼️ Chụp ảnh tĩnh / Snapshot"):
                    with gr.Row():
                        with gr.Column(scale=2):
                            webcam_input = gr.Image(label="📷 Chụp ảnh webcam hoặc Tải ảnh lên",
                                                    sources=["webcam", "upload"], type="numpy")
                            webcam_btn = gr.Button("🔍 Phân tích ảnh", variant="primary")
                        with gr.Column(scale=3):
                            webcam_out   = gr.Image(label="📸 Kết quả nhận diện khuôn mặt")
                ...
                # --- CHẾ ĐỘ 2: VIDEO CLIP (MỚI THÊM) ---
                with gr.Tab("📹 Quay Video Clip ngắn (3-5 giây)"):
                    with gr.Row():
                        with gr.Column(scale=2):
                            # Thành phần cho phép quay video trực tiếp bằng camera hoặc kéo thả file mp4
                            video_input = gr.Video(label="📹 Quay clip bằng camera hoặc kéo thả file Video", 
                                                   sources=["webcam", "upload"])
                            video_btn = gr.Button("🔍 Tiến hành phân tích đoạn Clip", variant="primary")
                        with gr.Column(scale=3):
                            video_label = gr.HTML()
                            video_bars  = gr.HTML()
                    video_suggestion = gr.HTML()

                    video_btn.click(fn=handle_video_input,
                                    inputs=[video_input, user_name_input],
                                    outputs=[video_label, video_bars, video_suggestion])
        ...
        with gr.Tab("🔀 Giọng nói + Khuôn mặt"):
            gr.HTML(
                ...
                '<div style="color:#a78bfa;font-size:13px;margin-bottom:4px">🔀 Học sâu Đa phương thức (Multimodal Late Fusion)</div>'
                '<div style="color:#8888aa;font-size:12px">'
                '💡 <b>Mẹo tối ưu:</b> Chỉ cần chọn tab <b>Video Clip</b>, bấm quay và nói tâm sự của bạn. Hệ thống sẽ tự động tách tiếng để nhận dạng chữ và phân tích chuỗi cơ mặt đồng thời!</div></div>'
            )
            with gr.Row():
                with gr.Column(scale=2):
                    gr.HTML('<div style="color:#a78bfa;font-weight:600;font-size:13px;margin-bottom:6px">📥 Lựa chọn 1: Sử dụng Video tích hợp (Khuyên dùng)</div>')
                    with gr.Tabs():
                        with gr.Tab("📹 Quay Video Clip ngắn (Có cả Tiếng + Hình)"):
                            mm_video = gr.Video(label="Bật camera quay clip 3-5s vừa nói vừa biểu cảm", sources=["webcam", "upload"])
                        
                        with gr.Tab("🖼️ Hoặc dùng linh hoạt Ảnh / Text rời (Luồng cũ)"):
                            mm_text  = gr.Textbox(label="💬 Nhập văn bản tâm trạng thủ công", lines=2, placeholder="Hôm nay tôi cảm thấy...")
                            mm_audio = gr.Audio(label="📁 Hoặc upload file âm thanh ghi âm sẵn", type="numpy", sources=["upload"])
                            mm_image = gr.Image(label="📷 Tải ảnh khuôn mặt tĩnh lên", sources=["webcam", "upload"], type="numpy")
                    
                    mm_btn = gr.Button("🔍 Bắt đầu Phân tích Đa phương thức", variant="primary")
                with gr.Column(scale=3):
                    gr.HTML('<div style="color:#a78bfa;font-weight:600;font-size:14px;margin-bottom:8px">🧠 Kết quả dung hợp hệ thống</div>')
                    mm_transcribed   = gr.Textbox(label="📝 Chuỗi văn bản bóc tách từ Video / Lời nói", interactive=False)
                    mm_emotion_label = gr.HTML()
                    mm_bars          = gr.HTML()
            mm_suggestion = gr.HTML()
            mm_score      = gr.HTML()

            # Khớp nối nút phân tích với đúng cấu trúc luồng dữ liệu 5 tham số đầu vào
            mm_btn.click(fn=handle_multimodal,
                         inputs=[mm_text, mm_audio, mm_image, mm_video, user_name_input],
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("  MENTAL HEALTH MONITOR — READY")
    print("="*60)
    print(f"  Text model  : ✅")
...
#   python mental_health_update_2_6.py